...
import SupplierOrderListRow from "./SupplierOrderListRow"
...
export default function SupplierOrderList({id}:OrderListProps) {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('/api/sorders', {
          method: 'POST',
          headers: {
            ...
            {isLoading ? (
              ...
              orders ? (
                orders.map((order: any) => (
                  <SupplierOrderListRow order={order} />
                ))
              ) : (
                <TableRow>
